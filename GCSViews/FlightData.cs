...
namespace MissionPlanner.GCSViews
{
    public partial class FlightData : MyUserControl, IActivate, IDeactivate
    {
        ...
        public string OCR(Image snapshot)
        {
            string text = "";
            return text;
        }

        public string Chatgpt(Image snapshot)
        {
            string text = "";
            return text;
        }

        public double[,] ReverseWaypoint(double[,] waypointtocurrent){

            int n = waypointtocurrent.GetLength(0);
            int m = waypointtocurrent.GetLength(1);

            double[,] waypointtocurrentnew = new double[n, m];

            for (int i = 0; i < n; i++)
            {
                for (int j = 0; j < m; j++)
                {
                    waypointtocurrentnew[i, j] = waypointtocurrent[n - i - 1, m - j - 1];
                }
            }

            return waypointtocurrentnew;
